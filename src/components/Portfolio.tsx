  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden rounded-3xl">
        ...
        {/* Bandeau haut pour titre (dark & light) */}
        <div className="portfolio-card-title-overlay" aria-hidden="true" />
      </div>

      {/* Title at top — blanc + ombre pour lisibilité (dark & light) */}
      <div className="portfolio-card-title-wrap absolute top-0 left-0 right-0 z-10">
        ...
        >
          ...
          >
            <button
              type="button"
              onClick={handleClose}
              className="absolute top-4 end-4 z-20 w-12 h-12 rounded-full bg-black/60 backdrop-blur flex items-center justify-center text-white hover:bg-black/80 transition-colors"
  ...
  return (
    <section id="portfolio" className="py-32 bg-background relative px-4">
      <div className="max-w-7xl mx-auto mb-12 px-4 md:px-0 flex flex-col items-center text-center">
        <h2 className="text-4xl md:text-6xl font-black font-outfit uppercase tracking-tight mb-4 text-foreground">
          {t.portfolio.title} <span className="text-neon-blue">{t.portfolio.titleHighlight}</span>
        </h2>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 px-4 md:px-0">
        {filteredRealisations.length > 0 ? (
          ...
          <div className="col-span-2 py-24 text-center text-muted">
            <p className="text-lg">{t.portfolio.empty}</p>
            <p className="text-sm mt-2 opacity-80">{t.portfolio.emptyHint}</p>
          </div>
        )}
      </div>
