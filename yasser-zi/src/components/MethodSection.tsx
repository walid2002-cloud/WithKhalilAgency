...
const Navbar = () => {
  return (
    ...
    >
      ...
      <button className="px-5 py-2 text-xs md:text-sm font-semibold bg-white text-black rounded-full hover:scale-[1.02] transition-transform duration-300">
        Démarrer
      </button>
    </motion.nav>
  );
...
const Card = ({ step, index }: { step: { id: string; title: string }; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden rounded-2xl bg-white/[0.03] border border-white/[0.06] p-6 md:p-8 backdrop-blur-md cursor-pointer transition-colors duration-500 hover:bg-white/[0.05]"
    >
  ...
  return (
    <div className="relative w-full bg-[#030303] selection:bg-white/20 font-sans">
      ...
      >
        ...
        {/* Title & Subtitle */}
        <div className="relative z-10 w-full flex flex-col items-center mb-6 md:mb-10 text-center">
          <motion.div
        ...
        <div className="relative z-10 w-full grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
